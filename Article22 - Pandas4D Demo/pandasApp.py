...
# Example 18: Applying multiple aggregate functions with groupby
print("Multiple aggregations based on geography, gender, and exited status:")
print(df[['Geography','Gender','Exited']].groupby(['Geography','Gender']).agg(['mean','count']))

# Example 19: Calculate the number of unique values in a column
print(f"Number of unique values in Geography: %s" % (df.Geography.value_counts().size))
print(f"Number of unique values in NumOfProducts: %s" % (df.NumOfProducts.value_counts().size))
print(f"Number of unique values in Tenure: %s" % (df.Tenure.value_counts().size))
print(f"Number of unique values in Age: %s" % (df.Age.value_counts().size))

# Example 20: The rank function
print("Create a new column that ranks the customers according to their balances:")
df['Rank'] = df['Balance'].rank(method='first', ascending=False).astype('int')
print(df)

# Example 21: Memory usage function
print("Memory usage:")
